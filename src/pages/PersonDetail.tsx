...
type Filter = 'all' | 'movie' | 'tv' | 'acting' | 'crew'
...
export default function PersonDetail() {
  ...
  const allCredits = useMemo(() => data ? dedupeCredits([...data.credits.cast.map(c => ({...c, department: 'Acting'})), ...data.credits.crew]) : [], [data])
  ...
  const filtered = useMemo(() => allCredits.filter(c => {
    ...
    if (filter === 'acting' && c.department !== 'Acting') return false
    if (filter === 'crew' && c.department === 'Acting') return false
